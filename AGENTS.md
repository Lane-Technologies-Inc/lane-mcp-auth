...
1. **Serve the protected-resource document at BOTH paths.**
   ...
   client a challenge pointing at a path you do not answer. The document's
   `scopes_supported` is what a client requests and what the consent screen
   offers: pass `scopesSupported` with every `<host>/<name>` tool permission you
   gate on, or those tools are refused for every host.
...
7. **Advertise the step-up tool and instruction** via `mergeTools()` and
   ...
   On the official SDK, `registerStepUpTool()` does this for you; on mcp-use,
   `withLaneAuth()` writes `server.config.instructions` for you. Everywhere
   else the instruction is yours to place, and skipping it is silent: the gate
   still refuses, and nothing on the wire says what to call instead.
