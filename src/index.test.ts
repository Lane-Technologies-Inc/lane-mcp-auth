...
describe('discovery', () => {
  ...
  // ── THE TWO PROPERTIES A PATH-FREE IDENTIFIER RELIES ON ────────────────────
  //
  // A server may publish its identifier WITHOUT the endpoint's `/mcp`, because a
  // hosted proxy that rewrites `resource` per well-known path otherwise gives
  // two documents that disagree about what the server is. One document cannot
  // disagree with itself, so that arrangement depends on `derived` being null
  // here -- the kind of thing a later refactor would "tidy" into an empty-string
  // path that silently resurrects the second document.
  it('has no derived path when the identifier carries none', () => {
    expect(metadataPaths('https://acme.example')).toEqual({
      root: '/.well-known/oauth-protected-resource',
      derived: null,
    });
  });

  // And the reason the rule is "strip the transport segment" rather than "use
  // the origin": a merchant serving MCP under a subpath must keep the prefix, or
  // its token audience widens from the MCP surface to the entire site.
  it('keeps a subpath prefix, so a shared host stays scoped', () => {
    expect(metadataPaths('https://merchant.example/shop')).toEqual({
      root: '/.well-known/oauth-protected-resource',
      derived: '/.well-known/oauth-protected-resource/shop',
    });
  });

  // ── THE ENDPOINT-SCOPED FORM, WHICH A PROXY MAY NAME INSTEAD ───────────────
  //
  // A gateway in front of this server can derive the challenge URL and each
  // document's `resource` from the TRANSPORT MOUNT rather than from `resource`.
  // mcp-use's OAuth proxy does. So `endpoint` must always be served, even when
  // it is not the form advertised, or the proxy points clients at a 404 they
  // cannot recover from (the SDK skips its root fallback for an explicitly
  // supplied metadataUrl).
  it('reports the endpoint-scoped path, which equals `derived` in the usual setup', async () => {
    const {auth} = await build();
    expect(auth.metadataPaths()).toEqual({
      root: '/.well-known/oauth-protected-resource',
      derived: '/.well-known/oauth-protected-resource/mcp',
  ...
  // The case this exists for: an identifier that carries no path, on a server
  // whose transport still lives at /mcp.
  it('still reports it when the identifier carries no path at all', async () => {
    const {auth} = await build({resource: 'https://acme.example'});
    expect(auth.metadataPaths()).toEqual({
      root: '/.well-known/oauth-protected-resource',
      derived: null,
      endpoint: '/.well-known/oauth-protected-resource/mcp',
  ...
  it('normalizes the configured mount, so `mcp` and `/mcp/` mean one path', async () => {
    for (const endpointPath of ['mcp', '/mcp/', '/mcp']) {
      const {auth} = await build({resource: 'https://acme.example', endpointPath});
      expect(auth.metadataPaths().endpoint).toBe('/.well-known/oauth-protected-resource/mcp');
    }
  });
  ...
  it('advertises the scopes the server names, verbatim', async () => {
    const {auth} = await build({
      scopesSupported: ['openid', 'email', 'shop.example/purchase'],
    });
    const doc = JSON.parse(auth.protectedResourceDocument());
    expect(doc.scopes_supported).toEqual(['openid', 'email', 'shop.example/purchase']);
  });
...
describe('tool merge', () => {
  ...
  it('tells a multi-server agent to reuse the SESSION, never the token', async () => {
    // The naive instruction -- "authenticate once, present the same token
    // everywhere" -- describes something the authorization server refuses by
    // construction: it sets `aud` to exactly one resource and rejects anything
    // else. An instruction that asked for it would send a model round a loop it
    // cannot exit.
    const text = stepUpInstruction();
    expect(text).toMatch(/session/i);
    expect(text).toMatch(/once on each server|once per server/i);
    expect(text).toMatch(/audience/i);
  });

  it('does not tell an agent to carry a token between servers', async () => {
    // A guard on the WORDING, because this is the one sentence whose failure
    // mode is a model doing exactly what it was told and getting a 401 forever.
    const text = stepUpInstruction().toLowerCase();
    expect(text).toContain('do not carry an access token from another server');
    expect(text).not.toMatch(/(same|one) (access )?token to (every|each|another|other)/);
  });
});
...
describe('createTokenExchanger', () => {
  ...
  it('posts the exchange to `${issuer}/token` with Basic auth', async () => {
    ...
    // The body is form-encoded; an AS that reads the header would otherwise
    // reject it as JSON.
...
describe('an expired connection', () => {
  ...
  it('is refused, not allowed on the strength of the row still existing', async () => {
    // The row survives its grant. Allowing on `if (record)` alone would let a
    // grant the authorization server issued for an hour authorize forever.
...
describe('the SHIPPED verification path', () => {
  ...
  // ── BOTH AUDIENCES, BECAUSE A PROXY PICKS WHICH ONE THE CLIENT READS ───────
  //
  // With a path-free identifier the server advertises `https://acme.example`,
  // but a fronting proxy rewrites the document it serves at the endpoint-scoped
  // path to `https://acme.example/mcp`. A client reading THAT one mints its
  // token for THAT audience. Refusing it would make token validity depend on
  // which document a vendor happened to show the client.
  it('accepts a token minted for the ENDPOINT-scoped audience', async () => {
    const {auth, privateKey} = await real({resource: 'https://acme.example'});
    const out = await auth.authenticate(
      await token(privateKey, {aud: 'https://acme.example/mcp'}),
    );
    expect(out.kind).toBe('ok');
  });

  it('accepts a token minted for the ADVERTISED audience', async () => {
    const {auth, privateKey} = await real({resource: 'https://acme.example'});
    const out = await auth.authenticate(await token(privateKey, {aud: 'https://acme.example'}));
    expect(out.kind).toBe('ok');
  });

  // Widening to two forms must not widen to a third. Same origin is not the
  // test -- the two accepted values are, and nothing else on the host is.
  it('still REFUSES another audience on the same origin', async () => {
    const {auth, privateKey} = await real({resource: 'https://acme.example'});
    const out = await auth.authenticate(
      await token(privateKey, {aud: 'https://acme.example/admin'}),
    );
    expect(out.kind).toBe('unauthenticated');
  });
