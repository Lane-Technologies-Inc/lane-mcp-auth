...
export {AUTH_GUIDE_NAME, AUTH_GUIDE_URI, authGuideText, type AuthGuideOptions} from './auth-guide.js';
 ...
 *  `scopesSupported`. Without this list an SDK client sends no `scope`, the
 *  consent screen offers nothing to choose, and a tool gated on `email` is
 *  refused for every host. No `phone`: web clients may not hold it. */
...
/**
 * What is appended to `initialize` instructions.
 *
 * ── THE SECOND PARAGRAPH IS ABOUT SESSIONS, NOT TOKENS, ON PURPOSE ──────────
 *
 * The obvious multi-server instruction -- "authenticate once and present the
 * same token to every Lane server" -- describes something that cannot work. A
 * Lane access token names exactly one audience, so a token minted for one
 * server presented at another MUST fail audience validation, and telling a
 * model otherwise sends it round a loop it cannot exit.
 *
 * What IS shared is one level up: a SESSION carries a list of audiences and
 * mints a per-audience token from each. So the instruction says reuse the
 * session and step up once per server -- which is true, and is also the
 * behaviour worth asking for, since the failure this text prevents is an agent
 * starting a fresh session per tool call.
 */
export function stepUpInstruction(): string {
  return (
    ...
    `session and call \`${STEP_UP_TOOL}\` once on each server — once per server, not ` +
...
export type LaneMcpAuth = {
  ...
  /** `root` is always served. `derived` is the RFC 9728 insertion form for a
   *  path-bearing `resource` (null when it carries none). `endpoint` is the
   *  form a fronting proxy derives from the transport mount — see
   *  `LaneMcpAuthConfig.endpointPath`. Serve all three; they may coincide. */
  metadataPaths(): {root: string; derived: string | null; endpoint: string};
...
export function createLaneMcpAuth(config: LaneMcpAuthConfig): LaneMcpAuth {
  ...
  const scopesSupported = [...(config.scopesSupported ?? DEFAULT_SCOPES_SUPPORTED)];
  const basePaths = metadataPaths(resource);
  const origin = new URL(resource).origin;

  // The transport mount, normalized to a leading slash and no trailing one, so
  // `/mcp`, `mcp` and `/mcp/` all describe the same endpoint.
  const endpointPath = `/${(config.endpointPath ?? '/mcp').replace(/^\/+|\/+$/g, '')}`;
  // What a proxy that derives discovery from the endpoint will name, and
  // therefore what this server must both SERVE and ACCEPT. Identical to
  // `resource` on the conventional setup where the identifier already carries
  // the transport segment; different only when it does not.
  const endpointResource = `${origin}${endpointPath}`;
  const paths = {
    ...basePaths,
    endpoint: `/.well-known/oauth-protected-resource${endpointPath}`,
  };
  const metadataUrl = `${origin}${paths.derived ?? paths.root}`;

  // Deduped: `jose` is happy with repeats, but the list is read by humans
  // debugging a refused token and a doubled entry reads as a bug.
  const audiences = [...new Set([canonicalResource, resource, endpointResource])];
      ...
      for (const iss of accepted) {
        try {
          const {payload, protectedHeader} = await jwtVerify(token, keysFor(iss), {
            issuer: iss,
            // Any-of. Lane's canonical resource and this server's own, both
            // accepted as a subject token at the exchange and NOT equally bound
            // (see the README) -- plus the endpoint-scoped form, because a
            // fronting proxy decides which of the two a client reads and the
            // token must validate either way.
            audience: audiences,
          });
