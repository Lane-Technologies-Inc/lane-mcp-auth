...
entrypoint. Same one call, same guarantees:

```ts
import {enableLaneAuth, mountDiscovery} from '@getonlane/mcp-auth/mcp-use';

enableLaneAuth(server, auth, {scopes: {place_order: 'email'}});
mountDiscovery(server, auth);           // the RFC 9728 document, both paths

// ...then declare tools normally. Every one is gated.
server.tool({name: 'place_order', schema: {sku: z.string()}}, placeOrder);
```

Three differences worth knowing, all of them mcp-use's shape rather than a
choice:
...
- The caller is found on the request context mcp-use passes each tool, which is
  ...
  will use them.
- A bad token comes back as a tool RESULT, not a 401. On the raw SDK you own the
  HTTP layer and can answer 401 yourself; here the adapter only learns inside the
  tool call, and a 401 from there tells a client to refresh a token that may be
  fine. `mountDiscovery` is what lets a client find out where to authenticate.

Tested against mcp-use 1.34.x. Its 2.x line has a different server surface;
until that is verified, treat it as unsupported rather than assumed.
...
grant is enforced against. The current limit is that **merchant-defined scopes
are not expressible at all yet**; until they are, gate tools on the scopes above,
or on connection existence alone.
