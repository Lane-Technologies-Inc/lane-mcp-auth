...
from lane_mcp_auth.fastmcp import enable_lane_auth, guarded, requires
...
async def test_requires_tag_refuses_when_the_connection_lacks_it() -> None:
    """A @requires tag the connection does not hold refuses the tool, naming it."""
    mcp = build_server(connected_scopes=[])  # connected, but holds no tags

    @mcp.tool()
    @requires("purchase")
    async def confirm_order(ctx=None) -> str:
        return "CHARGED"

    result = str(await confirm_order(ctx=Ctx(CLAIMS)))
    assert "CHARGED" not in result
    assert "insufficient_scope" in result
    assert "purchase" in result


async def test_requires_tag_allows_when_the_connection_holds_it() -> None:
    mcp = build_server(connected_scopes=["purchase"])
    ...
    assert str(await confirm_order(ctx=Ctx(CLAIMS))) == "ordered"


async def test_requires_needs_every_tag() -> None:
    """Two tags means BOTH; holding one is not enough."""
    mcp = build_server(connected_scopes=["purchase"])  # lacks `manage`

    @mcp.tool()
    @requires("purchase", "manage")
    async def modify_order(ctx=None) -> str:
        return "modified"

    result = str(await modify_order(ctx=Ctx(CLAIMS)))
    assert "modified" not in result
    assert "manage" in result


async def test_scopes_map_overrides_the_requires_tag() -> None:
    """build_server gates `place_order` on `payments:write`; the @requires tag on
    the same tool is overridden, so the deployment's requirement is what binds."""
    mcp = build_server(connected_scopes=["read"])
    ...
    result = str(await _handler(ctx=Ctx(CLAIMS)))
